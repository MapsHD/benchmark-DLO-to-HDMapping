...
# run_benchmark.sh - Reusable benchmark runner for DLO (ROS1)
set -e
...
echo "=========================================="
echo "Running DLO benchmark (ROS1)"
...
IMAGE="${DLO_IMAGE:-ghcr.io/mapshd/dlo2hdmapping:latest}"
...
docker run --rm -t \
    ...
    bash -lc "
      ...
      roslaunch ${DLO_LAUNCH} >/tmp/dlo.log 2>&1 &
      LAUNCH_PID=\$!
      ...
      rosrun dlo-to-hdmapping listener \"\${REC_BAG}\" \"\${OUT_DIR}/hdmapping\"
    "
